...
namespace dbr
{
    namespace eng
    {
        namespace gfx
        {
            uint32_t Color::asUintRGBA()
            {
                uint32_t ret = 0;

                ret |= static_cast<uint32_t>(r) << 24 & 0xff000000;
                ret |= static_cast<uint32_t>(g) << 16 & 0xff0000;
                ret |= static_cast<uint32_t>(b) << 8 & 0xff00;
                ret |= a;

                return ret;
            }

            uint32_t Color::asUintBGRA()
            {
                uint32_t ret = 0;

                ret |= static_cast<uint32_t>(b) << 24 & 0xff000000;
                ret |= static_cast<uint32_t>(g) << 16 & 0xff0000;
                ret |= static_cast<uint32_t>(r) << 8 & 0xff00;
                ret |= a;
